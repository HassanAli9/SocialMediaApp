...
class ApiService
{
    let header: HTTPHeaders = ["app-id": "629664d1a085f426eea3a3ae"]
    ...
    func getAllPosts(completion: @escaping (Post?, Error?, Bool) -> ())
    {
        ...
        AF.request(baseURL, headers: header).responseDecodable(of: Post.self)
        { response in
    ...
    func getCommentsForPost(postID: String, completion: @escaping (Comment?, Error?, Bool) -> ())
    {
        ...
        AF.request(commentUrl, headers: header).responseDecodable(of: Comment.self)
        { response in
            switch response.result
            {
            case .success:
    ...
    func getOwnerData(ownerId: String, completionHandler: @escaping (Owner?, Error?, Bool) -> ())
    {
        ...
        AF.request(ownerUrl, headers: header).responseDecodable(of: Owner.self)
        { response in
            switch response.result
            {
            case .success:
    ...
    func createUser(email: String, firstName: String, lastName: String, completion: @escaping (Owner?, String?, Bool) -> ())
    {
        ...
        AF.request(createUserURL, method: .post, parameters: user, encoder: JSONParameterEncoder.default, headers: header).validate().responseDecodable(of: Owner.self)
        { response in
    ...
    func getNewOwners(firstName: String, email: String, completionHandler: @escaping ([Owner]?, Error?, Bool) -> ())
    {
        ...
        AF.request(ownerUrl, parameters: param, encoder: URLEncodedFormParameterEncoder.default, headers: header).responseDecodable(of: [Owner].self)
        { response in
            switch response.result
            {
            case .success:
